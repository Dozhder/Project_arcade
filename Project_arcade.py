...
class Platformer(arcade.View):
    def __init__(self, map):
        ...
        self.score = 0
        self.batch = Batch()
        self.text_info = arcade.Text("WASD/стрелки — ходьба/лестницы • SPACE — прыжок • E — взаимодействие с рычагами",
    ...
    def on_update(self, dt: float):
        ...
        if arcade.check_for_collision_with_list(self.player, self.exit) and self.f_lever:
            con = sqlite3.connect('Saves.sqlite')
            if self.map == 0:
                time = con.cursor().execute('''SELECT level_test_time FROM results WHERE player_name = ?''',
                                            [PLAYER_NAME,]).fetchone()
                score = con.cursor().execute('''SELECT level_test_score FROM results WHERE player_name = ?''',
                                            [PLAYER_NAME,]).fetchone()
                if score is None or self.score > score[0]:
                    con.cursor().execute(f'''UPDATE results
                                    SET level_test_score = ?
                                    WHERE player_name = ?''', [self.score, str(PLAYER_NAME)])
                    con.commit()
                if time is None or self.timer < time[0] or time[0] == 0:
                    con.cursor().execute(f'''UPDATE results
                                    SET level_test_time = ?
                ...
                time = con.cursor().execute('''SELECT level_1_time FROM results WHERE player_name = ?''',
                                            [PLAYER_NAME,]).fetchone()
                score = con.cursor().execute('''SELECT level_1_score FROM results WHERE player_name = ?''',
                                            [PLAYER_NAME,]).fetchone()
                if score is None or self.score > score[0]:
                    con.cursor().execute(f'''UPDATE results
                                    SET level_1_score = ?
                ...
                time = con.cursor().execute('''SELECT level_2_time FROM results WHERE player_name = ?''',
                                            [PLAYER_NAME,]).fetchone()
                score = con.cursor().execute('''SELECT level_2_score FROM results WHERE player_name = ?''',
                                            [PLAYER_NAME,]).fetchone()
                if score is None or self.score > score[0]:
                    con.cursor().execute(f'''UPDATE results
                                    SET level_2_score = ?
